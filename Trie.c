...
void trie_insert(trie_t* trie, char* key, char* value) {
	...
	while (strlen(key) != 0) {
		...
		if (curr_node->children[first_letter] == NULL) {
			curr_node->children[first_letter] = trie_create_node(trie);
...
int __trie_remove(trie_t *trie, trie_node_t *curr_node, char *key) {
	...
	if (next_node != NULL && __trie_remove(trie, next_node, key + 1) == 1) {
